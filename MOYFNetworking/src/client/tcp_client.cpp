...
namespace MOYF {
    ...
    void TCPClient::Run() {
        io::async_connect(_socket, _endpoints, [this](const boost::system::error_code& ec, io::ip::tcp::endpoint ep) {
            if (!ec) {
                asyncRead();
            }
        });
    ...
    void TCPClient::asyncWrite() {
        io::async_write(_socket, io::buffer(_outgoingMessages.front()), [this](const boost::system::error_code ec, size_t bytesTransferred){
            onWrite(ec, bytesTransferred);
        });
    }

    void TCPClient::onWrite(const boost::system::error_code ec, size_t bytesTransferred) {
        if (ec) {
            Stop();
            return;
        }
